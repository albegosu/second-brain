"""The wiki: what remains of each capture, in Markdown and in English.
    ...
    wiki/sources/<yyyy-mm>/<id>-<slug>.jpg   what the capture looked like: a few frames in time order
...
def detach(cid: int):
    ...
    for old in (WIKI / "sources").glob(f"*/{cid:04d}-*.jpg"):
        old.unlink()
...
def build_index():
    ...
    for cat, about in CATEGORIES.items():
        ...
        for t in mine:
            n = len(t["sources"])
            uses = f" · used {used[t['key']]}×" if used[t["key"]] else ""
            lines.append(f"- [{t['title']}]({t['key']}.md) — {t['summary']} ({n} source{'' if n == 1 else 's'}{uses})")
            if t["patterns"]:
...
def file_capture(*, cid: int, captured: str, url: str, source: str, media: dict,
    ...
    sheet = p.contact_sheet(frames or [], (WIKI / rel).with_suffix(".jpg"))
    shown = (f"![{'Frames in time order, left to right and top to bottom' if media.get('kind') == 'video' else 'What was captured'}]"
             f"({sheet.name})\n\n") if sheet else ""
    write_page(WIKI / rel,
